...
This demo provides a simple local-only expense tracker with authentication (stored in `localStorage`), expense CRUD, charts, budget alerts, recurring reminders, filtering/search, and a dashboard. It is intentionally self-contained so you can open `index.html` directly in a browser or serve it with a simple HTTP server.
...
- Add / edit / delete expenses (date, amount in INR, category, description, recurrence weekly/monthly).
...
1. From the workspace root, start a simple HTTP server (recommended):
...
---
Generated/updated by the project maintainer tools.
